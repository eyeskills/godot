                        ...
                        yres(height)
{
    ...
    // frame stored as YUYV
    yuyv_frame.width = xres;
    yuyv_frame.height = yres;
    yuyv_frame.size = xres * yres * 2;
    yuyv_frame.data = (unsigned char *) malloc(yuyv_frame.size * sizeof(char));
...
Webcam::~Webcam()
{
      ...
      //free(yuyv_frame.data);
}

const YUYVImage& Webcam::frame(int timeout)
{
    for (;;) {
        fd_set fds;
        struct timeval tv;
...
bool Webcam::read_frame()
{
    ...
    yuyv_frame.data = (unsigned char *)buffers[buf.index].data;
